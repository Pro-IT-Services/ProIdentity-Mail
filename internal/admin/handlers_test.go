...
func TestBuildDomainDNSUsesSharedMailHostAndDomainAlias(t *testing.T) {
	...
	dns := buildDomainDNS(22, "customer.test", "", "", DNSSettings{
		MailHostname:    "mail.proidentity.cloud",
		AdminHostname:   "madmin.proidentity.cloud",
		WebmailHostname: "webmail.proidentity.cloud",
	})
	if dns.MailHost != "mail.proidentity.cloud" {
	...
	foundMX := false
	for _, record := range dns.Records {
		if record.Type == "MX" && record.Name == "customer.test" && record.Value == "mail.proidentity.cloud" && record.Priority != nil && *record.Priority == priority {
			foundMX = true
			break
		}
	}
	...
	if !hasRecord(dns.Records, "CNAME", "mail.customer.test", "mail.proidentity.cloud") {
		t.Fatalf("missing mail.customer.test alias to shared mail host: %+v", dns.Records)
	}
	if !hasProxiedRecord(dns.Records, "CNAME", "webmail.customer.test", "webmail.proidentity.cloud", true) {
		t.Fatalf("missing proxied webmail.customer.test alias: %+v", dns.Records)
	}
	if !hasProxiedRecord(dns.Records, "CNAME", "madmin.customer.test", "madmin.proidentity.cloud", true) {
...
func TestBuildDomainDNSUsesProxiedAddressRecordsForOwnWebAliases(t *testing.T) {
	dns := buildDomainDNS(22, "proidentity.cloud", "", "", DNSSettings{
		MailHostname:    "mail.proidentity.cloud",
		AdminHostname:   "madmin.proidentity.cloud",
		WebmailHostname: "webmail.proidentity.cloud",
		PublicIPv4:      "46.225.87.251",
	})

	if !hasProxiedRecord(dns.Records, "A", "webmail.proidentity.cloud", "46.225.87.251", true) {
		t.Fatalf("missing proxied webmail address record: %+v", dns.Records)
	}
	if !hasProxiedRecord(dns.Records, "A", "madmin.proidentity.cloud", "46.225.87.251", true) {
...
func TestCloudflarePlannerTreatsProxiedMailRecordsAsConflicts(t *testing.T) {
	desired := []cloudflareDNSRecord{
		{Type: "CNAME", Name: "mail.example.com", Content: "mail.proidentity.cloud", TTL: 1, Proxied: boolPtr(false)},
	}
	existing := []cloudflareDNSRecord{
		{ID: "proxied-mail", Type: "CNAME", Name: "mail.example.com", Content: "mail.proidentity.cloud", TTL: 1, Proxied: boolPtr(true)},
	}
